...
class NewsAgent(BaseAgent):
    ...
    def __init__(
        ...
        Settings.llm = Ollama(model=model_name, temperature=temperature, request_timeout=7*60)
    ...
    def _initialize_agent(self) -> ReActAgent:
        ...
        llm = Ollama(model=self.model_name, temperature=self.temperature, request_timeout=7*60)
        agent = ReActAgent(
            tools=tools,
            llm=llm,
            verbose=True,
    ...
    def _get_query_engine(self):
        if not hasattr(self, "_query_engine"):
            ...
            self._query_engine = VectorStoreIndex.from_vector_store(store).as_query_engine()
